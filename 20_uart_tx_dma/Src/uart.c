...
/* RM: Macro to define Mask value to enable bit5 of USART_CR1 register to check interrupt enabled for reception */
#define CR1_RXNEIE      (1U<<5)

/* RM: Macro to define Mask value to enable bit7 of USART_CR1 register to check interrupt enabled for transmission */
#define CR1_TXEIE       (1U<<7)

/* RM: Macro to define mask value to enable DMA1 module of AHB1ENR register*/
#define DMA1EN          (1U<<21)

/* RM: Macro to define mask value to select channel 4 of DMA1 module of DMA stream configuration register
 * (to select channel bit 25 = 0, bit 26 = 0, bit 27 = 1)
 * */
#define CHSEL4          (1U<<27)

/*RM: Macro to define mask value to enable memory increment of DMA1 module of DMA stream configuration register*/
#define DMA_MEM_INC     (1U<<10)

/*RM: Macro to define mask value to select direction from memory to peripheral of DMA1 module of DMA stream configuration register*/
#define DMA_DIR_MEM_TO_PERIPH (1U<<6)

/*RM: Macro to define mask value to select direction from memory to peripheral of DMA1 module of DMA stream configuration register*/
#define DMA_CR_TCIE     (1U<<4)

/*RM: Macro to define mask value to enable stream of DMA1 module of DMA stream configuration register*/
#define DMA_CR_EN       (1U<<0)

/*RM: Macro to define mask value to enable DMA Transmitter of USART2 module of USART control register 3*/
...
{
	/*RM: Enable clock access to DMA*/
	RCC->AHB1ENR |= DMA1EN;

	/*RM: Disable DMA1 stream6*/
	DMA1_Stream6->CR &=~ DMA_CR_EN;

	/*RM: Wait until DMA1 stream6 is disabled else struck*/
	while(DMA1_Stream6->CR & DMA_CR_EN){}

	/*RM: Clear all interrupt flags of stream6*/
	DMA1->HIFCR |= (1U<<16);
	DMA1->HIFCR |= (1U<<18);
	DMA1->HIFCR |= (1U<<19);
	DMA1->HIFCR |= (1U<<20);
	...
	/*RM: Set the destination buffer*/
	DMA1_Stream6->PAR = dst;

	/*RM: Set the source buffer*/
	DMA1_Stream6->M0AR = src;

	/*RM: Set the length*/
	DMA1_Stream6->NDTR = len;

	/*RM: Select stream6 CH4*/
	DMA1_Stream6->CR |= CHSEL4;

	/*RM: Enable memory increment*/
	DMA1_Stream6->CR |= DMA_MEM_INC;

	/*RM: Configure transfer direction*/
	DMA1_Stream6->CR |= DMA_DIR_MEM_TO_PERIPH;

	/*RM: Enable the DMA Transfer complete interrupt*/
	DMA1_Stream6->CR |= DMA_CR_TCIE;

	/*RM: Enable direct mode(bit2 = 0) and disable FIFO mode(all bits = 0)*/
	DMA1_Stream6->FCR = 0;

	/*RM: Enable DMA1 stream6*/
	DMA1_Stream6->CR |= DMA_CR_EN;

	/*RM: Enable UART2 transmitter DMA*/
    USART2->CR3 |= UART_CR3_DMAT;

	/*BSP: DMA interrupt enable in NVIC*/
...
void uart2_tx_interrupt_init(void)
{
	...
	/* RM: Configure the transfer direction and set all other bit to default by not using | operator */
	USART2->CR1 |= (CR1_TE | CR1_RE);
	/* RM: Enable UART2 Interrupt in NVIC */
	NVIC_EnableIRQ(USART2_IRQn);
	/* RM:Enable EXNE Interrupt */
 ...
 * dma1_stream6_init() function is used to transfer UART data in DMA mode
 *
 *  */
